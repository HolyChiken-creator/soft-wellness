...
export class TelegramState {
  ...
  async authorize(request) {
    ...
    if (!user || await sha256(secret) !== user.tokenHash) return null;
    return user;
  }
  ...
  async webhook(update) {
    ...
    if (startMatch) {
      ...
      await telegramCall(this.env, 'sendMessage', {
        chat_id: chatId,
        parse_mode: 'HTML',
        text: `✅ <b>Soft Wellness підключено.</b>\n\nБот працює автономно без AI та звертатиметься до вас як «${greeting(updated)}» відповідно до обраного в профілі пункту.\n\nКоманди: /status, /pause, /resume, /unlink.`
      });
      return json({ ok: true });
    }
