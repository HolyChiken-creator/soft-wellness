...
export class TelegramState {
  ...
  async webhook(update) {
    ...
    if (/^\/start\b/i.test(command) || /^\/connect\b/i.test(command) || /^\/login\b/i.test(command)) {
      const code = await this.createPwaClaim(user);
      const ticket = await this.createLaunchTicket(user);
      await telegramCall(this.env, 'sendMessage', {
        chat_id: chatId,
        parse_mode: 'HTML',
        text: `✅ <b>Ваш аккаунт Soft Wellness готов</b>

Telegram теперь является основной авторизацией. Mini App всегда открывает одну и ту же сессию.

Чтобы подключить установленную PWA на телефоне:
1. откройте в PWA «Профиль → Telegram»;
2. введите код ниже;
3. подтвердите перенос данных.

Код входа: <code>${code}</code>
Действует 10 минут и используется один раз.

ID общей сессии: <code>${String(user.id).slice(0, 8).toUpperCase()}</code>.`,
        reply_markup: user.appUrl ? {
    ...
    if (/^\/code\b/i.test(command)) {
      const code = await this.createPwaClaim(user);
      await telegramCall(this.env, 'sendMessage', {
        chat_id: chatId,
        parse_mode: 'HTML',
        text: `Код входа для PWA: <code>${code}</code>

Он действует 10 минут и используется один раз.`
      });
      return json({ ok: true });
    }
    ...
    if (/^\/pause\b/i.test(command)) {
      ...
      user.updatedAt = new Date().toISOString();
      await this.state.storage.put(`user:${user.id}`, user);
      await telegramCall(this.env, 'sendMessage', { chat_id: chatId, text: '⏸ Напоминания приостановлены. Для восстановления отправьте /resume.' });
    } else if (/^\/resume\b/i.test(command)) {
      user.enabled = true;
      user.updatedAt = new Date().toISOString();
      await this.state.storage.put(`user:${user.id}`, user);
      await telegramCall(this.env, 'sendMessage', { chat_id: chatId, text: '▶️ Напоминания восстановлены.' });
    } else if (/^\/unlink\b/i.test(command)) {
      await telegramCall(this.env, 'sendMessage', { chat_id: chatId, text: 'Telegram остаётся вашим аккаунтом. Чтобы отключить PWA на конкретном устройстве, выйдите из Telegram-сессии в настройках самой PWA.' });
    } else if (/^\/status\b/i.test(command)) {
      await telegramCall(this.env, 'sendMessage', {
        chat_id: chatId,
        parse_mode: 'HTML',
        text: `Soft Wellness: <b>${user.enabled ? 'напоминания активны' : 'напоминания приостановлены'}</b>.
Часовой пояс: ${escapeHtml(user.timezone)}.
Цель воды: ${Math.round(user.waterGoalMl)} мл.
Общая сессия: <code>${String(user.id).slice(0, 8).toUpperCase()}</code>.

Новый код для PWA: /code`
      });
    } else {
      await telegramCall(this.env, 'sendMessage', { chat_id: chatId, text: 'Команды: /code, /status, /pause, /resume.' });
    }
    return json({ ok: true });
  }
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/api/telegram/config') {
      return json({ botUsername: text(env.TELEGRAM_BOT_USERNAME, 80).replace(/^@/, ''), telegramFirst: true });
    }
