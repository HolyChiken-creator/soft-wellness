      ...
      if (!code) {
        ...
        if (existing?.appUrl) {
          ...
          await telegramCall(this.env, 'sendMessage', {
            chat_id: chatId,
            parse_mode: 'HTML',
            text: `✅ <b>Авторизація вже активна</b>

Telegram прив’язаний до сесії <code>${String(existing.id).slice(0, 8).toUpperCase()}</code>. Кнопка нижче відкриє саме цей профіль і той самий щоденник.`,
            reply_markup: {
      ...
      await telegramCall(this.env, 'sendMessage', {
        chat_id: chatId,
        parse_mode: 'HTML',
        text: `✅ <b>Авторизацію успішно підтверджено</b>

Ви підключили Telegram до Soft Wellness.

<b>Що сталося:</b>
• бот прив’язаний до вашого профілю;
• PWA та Telegram Mini App відкриватимуть один щоденник;
• ідентифікатор спільної сесії: <code>${String(updated.id).slice(0, 8).toUpperCase()}</code>;
• нагадування надходитимуть за вашим розкладом.

Бот працює автономно без AI та звертатиметься до вас як «${greeting(updated)}».

Натисніть кнопку нижче, щоб перевірити спільну сесію.
