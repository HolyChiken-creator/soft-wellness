...
export class TelegramState {
  ...
  async telegramSession(payload) {
    ...
    let user = await this.userByChat(telegramUserId);
    const requestedCode = String(payload?.startParam || verified.startParam || '').replace(/^link[_-]?/i, '').toUpperCase();
    if (!user && /^[A-Z0-9]{6,12}$/.test(requestedCode)) {
      ...
      if (candidate && Number(candidate.linkExpiresAt) > Date.now()) {
        user = { ...candidate, chatId: telegramUserId, enabled: true, connectedAt: new Date().toISOString(), linkCode: '', linkExpiresAt: 0 };
        await this.state.storage.put(`user:${user.id}`, user);
        await this.state.storage.delete(`link:${requestedCode}`);
      }
    }
    if (!user) {
      return json({ error: 'Сначала подключите бота кодом из PWA, затем откройте Mini App кнопкой в чате.' }, 404);
    }
  ...
  async userByChat(chatId) {
    ...
    for (const user of users.values()) {
      if (String(user.chatId || '') === String(chatId)) return user;
    }
    return null;
  }
  ...
  async webhook(update) {
    ...
    if (startMatch) {
      ...
      if (!code) {
        ...
        if (existing?.appUrl) {
          await telegramCall(this.env, 'sendMessage', {
            chat_id: chatId,
            text: 'Soft Wellness уже підключено. Відкрийте застосунок кнопкою нижче — це буде та сама синхронізована сесія, що й у PWA.',
            reply_markup: {
              inline_keyboard: [[{
                text: '🌿 Відкрити Soft Wellness',
                web_app: { url: `${existing.appUrl}/?telegram=1` }
              }]]
            }
          });
        } else {
      ...
      const updated = { ...user, chatId: String(chatId), enabled: true, connectedAt: new Date().toISOString(), linkCode: '', linkExpiresAt: 0, updatedAt: new Date().toISOString() };
      await this.state.storage.put(`user:${user.id}`, updated);
...
Команди: /status, /pause, /resume, /unlink.`,
        reply_markup: updated.appUrl ? {
          inline_keyboard: [[{
            text: '🌿 Відкрити Soft Wellness',
            web_app: { url: `${updated.appUrl}/?telegram=1` }
          }]]
        } : undefined
      });
      return json({ ok: true });
