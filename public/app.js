(() => {
  ...
  const STORAGE_KEY = 'softWellnessStateV8';
  const LEGACY_STORAGE_KEYS = ['softWellnessStateV7', 'softWellnessStateV6', 'softWellnessStateV5', 'softWellnessStateV4'];
  const APP_VERSION = 8;
  const MEALS = [
  ...
  async function initializeTelegramMiniAppSession() {
    ...
    try {
      ...
      renderTelegramStatus();
      startSessionPolling();
      toast('Открыта та же сессия, что и в PWA');
      return true;
    } catch (error) {
  ...
  function openTelegramReminders() {
    ...
    $('#openSameTelegramSession')?.addEventListener('click', async () => {
      try {
    ...
    const registerOrRefresh = async () => {
      ...
      try {
        ...
        closeModal();
        toast('Код подключения и общая сессия созданы');
    ...
    $('#saveTelegramSettings').addEventListener('click', async () => {
      ...
      try {
        if (!state.telegram.authToken) {
          ...
          await pushAppState({ quiet: false });
          closeModal();
          toast('Настройки и общая сессия сохранены');
          setTimeout(openTelegramReminders, 180);
          return;
        }
        const result = await telegramApi('/api/telegram/settings', { method: 'PUT', body: payload });
    ...
    refreshTelegramStatus({ quiet: true }).then((status) => {
      ...
      card.querySelector('small').textContent = status.connected ? 'PWA и Telegram Mini App используют одну облачную сессию.' : 'Отправьте боту команду /start с кодом подключения.';
    });
  }
