(() => {
  ...
  const STORAGE_KEY = 'softWellnessStateV11';
  const DEVICE_KEY = 'softWellnessDeviceV1';
  const LEGACY_STORAGE_KEYS = ['softWellnessStateV10', 'softWellnessStateV9', 'softWellnessStateV8', 'softWellnessStateV7', 'softWellnessStateV6', 'softWellnessStateV5', 'softWellnessStateV4'];
  ...
  function getDeviceIdentity(kind = telegramMiniApp ? 'telegram-mini-app' : 'pwa') {
    let stored = null;
    try { stored = JSON.parse(localStorage.getItem(DEVICE_KEY) || 'null'); } catch {}
    const id = stored?.id || (crypto.randomUUID ? crypto.randomUUID() : `${Date.now()}-${Math.random().toString(16).slice(2)}`);
    const userAgent = navigator.userAgent || '';
    const platform = /iPhone|iPad|iPod/i.test(userAgent) ? 'iPhone'
      : /Android/i.test(userAgent) ? 'Android'
        : /Mac/i.test(userAgent) ? 'Mac'
          : /Windows/i.test(userAgent) ? 'Windows'
            : 'Телефон';
    const name = kind === 'telegram-mini-app' ? `${platform} · Telegram Mini App` : `${platform} · PWA`;
    const identity = { id, name, kind };
    try { localStorage.setItem(DEVICE_KEY, JSON.stringify({ id })); } catch {}
    return identity;
  }
  ...
  function telegramDevicesMarkup(devices = state.telegram.devices || []) {
    if (!Array.isArray(devices) || !devices.length) {
      return '<div class="telegram-device-empty">Ще немає підключених PWA.</div>';
    }
    return `<div class="telegram-device-list">${devices.map((device, index) => `<div class="telegram-device-row"><span>📱</span><div><strong>PWA №${index + 1} · ${escapeHtml(device.name || 'Телефон')}</strong><small>Остання активність: ${escapeHtml(formatDeviceSeen(device.lastSeenAt))}</small></div><i>✓</i></div>`).join('')}</div>`;
  }
  ...
  let statePullInterval = null;
  let applyingRemoteState = false;
  let telegramMiniApp = false;
  let telegramAuthorizationWatchTimer = null;
  let telegramAuthorizationWatchDeadline = 0;
  ...
  function persist(showToast = true) {
    ...
    if (!applyingRemoteState) scheduleStateSync();
  ...
  function navigate(screen) {
    if (screen !== 'add' || addTab !== 'barcode') stopScanner();
  ...
  function scannerErrorMessage(error) {
    ...
    if (name === 'NotAllowedError' || name === 'PermissionDeniedError') return 'Доступ к камере запрещён. Разрешите камеру в настройках Safari, браузера или Telegram.';
    if (name === 'NotFoundError' || name === 'DevicesNotFoundError') return 'Основная камера не найдена.';
    if (name === 'NotReadableError' || name === 'TrackStartError') return 'Камера занята другим приложением. Закройте его и повторите.';
    if (!window.isSecureContext) return 'Камера работает только на HTTPS-адресе.';
    return 'Не удалось запустить камеру. Попробуйте режим «Сфотографировать штрихкод».';
  }

  async function acceptScannedBarcode(rawValue) {
    const barcode = String(rawValue || '').replace(/\D/g, '');
    if (barcode.length < 8 || barcode.length > 18) return false;
    $('#barcodeInput').value = barcode;
    stopScanner();
    setScannerStatus(`Код ${barcode} распознан`, 'success');
    await lookupBarcode();
    return true;
  }
  ...
  async function startNativeBarcodeScanner(video) {
    ...
    const supported = typeof BarcodeDetector.getSupportedFormats === 'function'
      ...
      : wanted;
    const formats = wanted.filter((formatName) => supported.includes(formatName));
    ...
    const scanFrame = async () => {
      ...
      if (codes.length && await acceptScannedBarcode(codes[0].rawValue)) return;
      if ('requestVideoFrameCallback' in video) video.requestVideoFrameCallback(scanFrame);
    ...
    for (const src of sources) {
      try {
        await new Promise((resolve, reject) => {
          const existing = document.querySelector(`script[data-zxing-src="${src}"]`);
          if (existing) {
            existing.addEventListener('load', resolve, { once: true });
            existing.addEventListener('error', reject, { once: true });
            setTimeout(() => window.ZXingBrowser ? resolve() : reject(new Error('ZXing timeout')), 4500);
            return;
          }
  ...
  async function startZxingScanner(video) {
    if (!await ensureZxing()) throw new Error('ZXING_UNAVAILABLE');
    ...
    scannerControls = await scannerReader.decodeFromConstraints(
      ...
      async (result) => {
        ...
        await acceptScannedBarcode(value);
      }
    );
    video.classList.remove('is-hidden');
    return true;
  ...
  async function startScanner() {
    if (scannerRunning) {
      stopScanner();
      setScannerStatus('Сканирование остановлено');
      return;
    }
    if (!window.isSecureContext) {
      setScannerStatus('Камера доступна только на HTTPS-адресе.', 'error');
      toast('Откройте опубликованный HTTPS-сайт');
      return;
    }
    if (!navigator.mediaDevices?.getUserMedia) {
      setScannerStatus('Этот браузер не даёт веб-приложению доступ к камере.', 'error');
      $('#scannerPhotoInput')?.click();
      return;
    }
    const video = $('#scannerVideo');
    scannerRunning = true;
    $('#scanCamera').textContent = 'Остановить';
    setScannerStatus('Разрешите доступ к камере и наведите её на штрихкод…');
    try {
      const nativeStarted = await startNativeBarcodeScanner(video);
      if (!nativeStarted) await startZxingScanner(video);
      setScannerStatus('Наведите штрихкод в центр кадра. Распознавание произойдёт автоматически.');
    } catch (error) {
  ...
  async function scanBarcodePhoto(file) {
    if (!file) return;
    setScannerStatus('Распознаём штрихкод на фото…');
    try {
      let rawValue = '';
      ...
      if (!rawValue && await ensureZxing()) {
        const reader = new window.ZXingBrowser.BrowserMultiFormatReader();
        const objectUrl = URL.createObjectURL(file);
        try {
          const result = await reader.decodeFromImageUrl(objectUrl);
          rawValue = typeof result?.getText === 'function' ? result.getText() : result?.text || '';
        } finally {
          URL.revokeObjectURL(objectUrl);
        }
      }
      if (!await acceptScannedBarcode(rawValue)) throw new Error('BARCODE_NOT_FOUND');
    } catch (error) {
      console.warn('Photo barcode scan failed', error);
      setScannerStatus('Штрихкод на фото не распознан. Снимите его крупнее, без бликов и строго прямо.', 'error');
      toast('Не удалось распознать штрихкод');
    }
  }
  ...
  function openOnboardingModal() {
    ...
    openModal(`<div class="sheet__handle"></div>
      ...
      <div class="welcome-privacy"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 3l7 3v5c0 4.5-2.8 8.4-7 10-4.2-1.6-7-5.5-7-10V6l7-3Z"/><path d="M9.4 12.2l1.7 1.7 3.7-4"/></svg><span>Без Telegram данные остаются только на этом устройстве.</span></div>`);
    renderSteps();
    const finish = () => {
  ...
  function renderTelegramStatus() {
    ...
    if (!element) return;
    const count = Number(state.telegram.deviceCount || state.telegram.devices?.length || 0);
    if (state.telegram.connected && state.telegram.enabled) element.textContent = `✅ Общая сессия · PWA: ${count || 1} · напоминания активны`;
    else if (state.telegram.connected) element.textContent = `✅ Общая сессия · PWA: ${count || 1} · напоминания приостановлены`;
    else element.textContent = 'Войти через Telegram';
  }
  ...
  function telegramHaptic(type = 'success') {
    ...
    try {
      if (navigator.vibrate) navigator.vibrate(type === 'success' ? [80, 45, 120] : [150]);
    } catch {}
  }

  function telegramStatusCopy(status = state.telegram) {
    if (status.connected) {
      return {
        title: '✅ Авторизация подтверждена',
        description: 'Один Telegram-аккаунт используется в PWA и Mini App.',
        className: 'is-connected'
      };
    }
    if (state.telegram.awaitingAuthorization) {
      return {
    ...
    return {
      title: 'Вход выполняется через Telegram',
      description: 'Сначала запустите бота, затем введите одноразовый код в PWA.',
      className: ''
    };
  }

  function updateTelegramStatusCard(status = state.telegram) {
    const card = $('#telegramStatusCard');
    if (!card) return;
    const copy = telegramStatusCopy(status);
    card.classList.toggle('is-connected', Boolean(status.connected));
    card.classList.toggle('is-pending', !status.connected && Boolean(state.telegram.awaitingAuthorization));
    card.querySelector('strong').textContent = copy.title;
    card.querySelector('small').textContent = copy.description;
  }

  async function openExactTelegramSession() {
    try {
      const launch = await telegramApi('/api/telegram/launch-ticket', { method: 'POST' });
      if (!launch.url) throw new Error('Не удалось создать ссылку на общую сессию');
      location.href = launch.url;
    } catch (error) {
      toast(error.message || 'Не удалось открыть Telegram');
    }
  }

  function showTelegramAuthorizationSuccess({ source = 'pwa', force = false } = {}) {
    const sessionId = state.telegram.sessionId || '';
    if (!force && sessionId && state.telegram.authorizationAcknowledgedSessionId === sessionId) return;

    state.telegram.awaitingAuthorization = false;
    state.telegram.authStartedAt = '';
    state.telegram.authorizationAcknowledgedSessionId = sessionId;
    persist(false);
    renderTelegramStatus();
    telegramHaptic('success');

    const sourceText = source === 'telegram'
      ? 'Telegram подтвердил личность и открыл ваш постоянный аккаунт.'
      : 'Эта PWA вошла в аккаунт, который был создан со стороны Telegram.';
    const pwaCount = Number(state.telegram.deviceCount || state.telegram.devices?.length || 0);

    openModal(`<div class="sheet__handle"></div>
      <div class="auth-success">
        <div class="auth-success__icon" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M5 12.5l4.2 4.2L19 7"/></svg></div>
        <p class="auth-success__eyebrow">TELEGRAM CONNECTED</p>
        <h2>Авторизация <em>успешна</em></h2>
        <p>${escapeHtml(sourceText)}</p>
        <div class="auth-success__session"><small>ID общей сессии</small><strong>${escapeHtml(sessionId ? sessionId.slice(0, 8).toUpperCase() : 'SYNCED')}</strong></div>
        <ul class="auth-success__list">
          <li><span>✓</span> Один профиль и рацион в PWA и Mini App</li>
          <li><span>✓</span> Підключено PWA: ${pwaCount || (telegramMiniApp ? 0 : 1)}</li>
          <li><span>✓</span> Изменения синхронизируются автоматически</li>
          <li><span>✓</span> Бот будет отправлять выбранные напоминания</li>
        </ul>
        <div class="sheet__actions ${telegramMiniApp ? 'sheet__actions--single' : ''}">
          ${telegramMiniApp ? '' : '<button id="authSuccessOpenTelegram" class="secondary-button" type="button">Открыть Telegram</button>'}
          <button id="authSuccessDone" class="primary-button" type="button">${telegramMiniApp ? 'Продолжить в дневник' : 'Понятно'}</button>
        </div>
      </div>`);

    $('#authSuccessDone')?.addEventListener('click', () => {
      closeModal();
      navigate('diary');
      toast('✅ Общая сессия активна');
    });
    $('#authSuccessOpenTelegram')?.addEventListener('click', openExactTelegramSession);
  }

  function stopTelegramAuthorizationWatch() {
    clearTimeout(telegramAuthorizationWatchTimer);
    telegramAuthorizationWatchTimer = null;
    telegramAuthorizationWatchDeadline = 0;
  }

  function startTelegramAuthorizationWatch({ durationMs = 120000 } = {}) {
    stopTelegramAuthorizationWatch();
    state.telegram.awaitingAuthorization = true;
    state.telegram.authStartedAt = new Date().toISOString();
    persist(false);
    renderTelegramStatus();
    updateTelegramStatusCard();

    telegramAuthorizationWatchDeadline = Date.now() + durationMs;

    const check = async () => {
      const status = await refreshTelegramStatus({ quiet: true, announce: true });
      if (status?.connected) {
        stopTelegramAuthorizationWatch();
        return;
      }
      if (Date.now() >= telegramAuthorizationWatchDeadline) {
        stopTelegramAuthorizationWatch();
        state.telegram.awaitingAuthorization = false;
        persist(false);
        renderTelegramStatus();
        updateTelegramStatusCard();
        toast('Код всё ещё действителен. Вернитесь после команды /start в боте');
        return;
      }
      telegramAuthorizationWatchTimer = setTimeout(check, 2500);
    };

    telegramAuthorizationWatchTimer = setTimeout(check, 1200);
  }
  ...
  async function telegramApi(path, { method = 'GET', body = null } = {}) {
    ...
    headers['x-soft-wellness-device-id'] = device.id;
    headers['x-soft-wellness-device-name'] = encodeURIComponent(device.name);
    headers['x-soft-wellness-device-kind'] = device.kind;
    ...
    if (!response.ok) {
      const error = new Error(payload.error || 'Telegram-сервис временно недоступен');
  ...
  function storeStateSilently() {
    state.version = APP_VERSION;
    localStorage.setItem(STORAGE_KEY, JSON.stringify(state));
  }

  function applyRemoteAppState(remoteState, metadata = {}) {
    if (!remoteState || typeof remoteState !== 'object') return false;
    const fallback = defaultState();
    const localTelegram = state.telegram || fallback.telegram;
    applyingRemoteState = true;
    try {
      const profile = { ...fallback.profile, ...(remoteState.profile || {}) };
      const merged = {
        ...fallback,
        ...remoteState,
        profile,
        goals: { ...calculateGoals(profile.weight, profile.waterGoalMl), ...(remoteState.goals || {}) },
        days: remoteState.days || {},
        customFoods: Array.isArray(remoteState.customFoods) ? remoteState.customFoods : [],
        recentFoodIds: Array.isArray(remoteState.recentFoodIds) ? remoteState.recentFoodIds : fallback.recentFoodIds,
        weightHistory: Array.isArray(remoteState.weightHistory) ? remoteState.weightHistory : [],
        telegram: {
          ...fallback.telegram,
          ...(remoteState.telegram || {}),
          authToken: localTelegram.authToken,
          sessionId: localTelegram.sessionId || metadata.sessionId || '',
          connected: Boolean(metadata.connected ?? localTelegram.connected),
          enabled: Boolean(metadata.enabled ?? remoteState.telegram?.enabled ?? localTelegram.enabled),
          botUsername: metadata.botUsername || localTelegram.botUsername || '',
          linkCode: localTelegram.linkCode || '',
          syncRevision: Number(metadata.revision || remoteState.telegram?.syncRevision || 0),
          lastSyncedAt: metadata.updatedAt || new Date().toISOString(),
          syncReady: true,
          deviceCount: Number(metadata.deviceCount ?? localTelegram.deviceCount ?? 0),
          devices: Array.isArray(metadata.devices) ? metadata.devices : (Array.isArray(localTelegram.devices) ? localTelegram.devices : [])
        },
        ui: { ...fallback.ui, ...(remoteState.ui || {}) }
      };
      state = merged;
      storeStateSilently();
    } finally {
  ...
  async function pushAppState({ quiet = true } = {}) {
    if (!state.telegram.authToken) return null;
    try {
      const result = await telegramApi('/api/sync/state', {
        method: 'PUT',
        body: {
          state: stateForCloud(),
          baseRevision: Number(state.telegram.syncRevision || 0),
          source: telegramMiniApp ? 'telegram-mini-app' : 'pwa'
        }
      });
      state.telegram.syncRevision = Number(result.revision || state.telegram.syncRevision || 0);
      state.telegram.lastSyncedAt = result.updatedAt || new Date().toISOString();
      state.telegram.syncReady = true;
      storeStateSilently();
      return result;
    } catch (error) {
      if (!quiet) toast(error.message || 'Не удалось синхронизировать данные');
      console.warn('State push failed', error);
      return null;
    }
  }

  function scheduleStateSync() {
    if (!state.telegram.authToken || applyingRemoteState) return;
    clearTimeout(stateSyncTimer);
    stateSyncTimer = setTimeout(() => pushAppState({ quiet: true }), 850);
  }

  async function pullAppState({ force = false, quiet = true } = {}) {
    if (!state.telegram.authToken) return null;
    try {
      const result = await telegramApi('/api/sync/state');
      state.telegram.deviceCount = Number(result.deviceCount || result.devices?.length || state.telegram.deviceCount || 0);
      state.telegram.devices = Array.isArray(result.devices) ? result.devices : state.telegram.devices;
      if (!result.state) {
        storeStateSilently();
        await pushAppState({ quiet: true });
        return null;
      }
      const remoteRevision = Number(result.revision || 0);
      const localRevision = Number(state.telegram.syncRevision || 0);
      if (force || remoteRevision > localRevision || !state.telegram.syncReady) {
        applyRemoteAppState(result.state, result);
      }
      return result;
    } catch (error) {
      if (!quiet) toast(error.message || 'Не удалось загрузить общую сессию');
      console.warn('State pull failed', error);
      return null;
    }
  }

  async function initializeTelegramMiniAppSession() {
    const webApp = window.Telegram?.WebApp;
    const initData = webApp?.initData || '';
    telegramMiniApp = Boolean(initData);
    if (!initData) return false;
    showSessionGate('Входим через Telegram…');
    try {
      const response = await fetch('/api/telegram/session', {
        method: 'POST',
        headers: { 'content-type': 'application/json', accept: 'application/json' },
        body: JSON.stringify({ initData, appUrl: location.origin, device: getDeviceIdentity('telegram-mini-app') })
      });
      const result = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(result.error || 'Не удалось открыть общую сессию');
      state.telegram.authToken = result.authToken;
      state.telegram.sessionId = result.sessionId || '';
      state.telegram.connected = true;
      state.telegram.enabled = result.enabled !== false;
      state.telegram.botUsername = result.botUsername || state.telegram.botUsername;
      state.telegram.syncRevision = Number(result.revision || 0);
      state.telegram.syncReady = false;
      state.telegram.awaitingAuthorization = false;
      state.telegram.deviceCount = Number(result.deviceCount || 0);
      state.telegram.devices = Array.isArray(result.devices) ? result.devices : [];
      storeStateSilently();

      // The server returns the exact PWA snapshot in the session response. Apply it before
      // showing the UI, so Telegram never flashes a separate WebView localStorage session.
      if (result.state) {
        applyRemoteAppState(result.state, result);
      } else {
        const telegramCredentials = { ...state.telegram };
        state = defaultState();
        state.telegram = {
          ...state.telegram,
          ...telegramCredentials,
        ...
        renderProfile();
        renderFoodCatalog();
        await pushAppState({ quiet: false });
      }
      renderTelegramStatus();
      startSessionPolling();
      hideSessionGate();
      setTimeout(() => showTelegramAuthorizationSuccess({ source: 'telegram', force: true }), 120);
      return true;
    } catch (error) {
      console.warn('Telegram Mini App session failed', error);
      toast(error.message || 'Не удалось синхронизировать Telegram Mini App');
      return false;
    } finally {
      hideSessionGate();
    }
  ...
  function scheduleTelegramSync() {
    if (!state.telegram.authToken || !state.telegram.connected) return;
  ...
  function applyTelegramSettings(payload, server = {}) {
    state.telegram = {
      ...
      sessionId: server.sessionId || state.telegram.sessionId || '',
  ...
  async function refreshTelegramStatus({ quiet = false, announce = false } = {}) {
    if (!state.telegram.authToken) return null;
    try {
      const wasConnected = Boolean(state.telegram.connected);
      const previousSessionId = state.telegram.sessionId || '';
      const status = await telegramApi('/api/telegram/status');
      state.telegram.sessionId = status.sessionId || state.telegram.sessionId || '';
      ...
      state.telegram.deviceCount = Number(status.deviceCount || status.devices?.length || 0);
      state.telegram.devices = Array.isArray(status.devices) ? status.devices : state.telegram.devices;
      if (state.telegram.connected) state.telegram.awaitingAuthorization = false;
      persist(false);
      renderTelegramStatus();
      updateTelegramStatusCard(status);

      const sessionChanged = Boolean(state.telegram.sessionId && state.telegram.sessionId !== previousSessionId);
      const needsFeedback = state.telegram.connected
        && (announce || !wasConnected || sessionChanged)
        && state.telegram.authorizationAcknowledgedSessionId !== state.telegram.sessionId;

      if (needsFeedback) {
        stopTelegramAuthorizationWatch();
        setTimeout(() => showTelegramAuthorizationSuccess({ source: telegramMiniApp ? 'telegram' : 'pwa' }), 80);
      }
      return status;
    } catch (error) {
  ...
  function hasMeaningfulLocalData(snapshot = state) {
    const profile = snapshot.profile || {};
    const days = snapshot.days || {};
    const hasDiary = Object.values(days).some((day) => ['breakfast', 'lunch', 'dinner', 'snack'].some((meal) => Array.isArray(day?.[meal]) && day[meal].length) || Number(day?.waterMl) > 0);
    return Boolean(snapshot.ui?.profileComplete || profile.name || hasDiary || snapshot.weightHistory?.length || snapshot.customFoods?.length);
  }

  function applyClaimCredentials(result) {
    state.telegram.authToken = result.authToken;
    state.telegram.sessionId = result.sessionId || '';
    state.telegram.connected = true;
    state.telegram.enabled = result.enabled !== false;
    ...
    state.telegram.syncReady = false;
    state.telegram.awaitingAuthorization = false;
    state.telegram.authorizationAcknowledgedSessionId = result.sessionId || '';
    state.telegram.deviceCount = Number(result.deviceCount || result.devices?.length || 0);
    state.telegram.devices = Array.isArray(result.devices) ? result.devices : [];
    storeStateSilently();
  }

  async function finalizeTelegramClaim(result, mode) {
    const localSnapshot = JSON.parse(JSON.stringify(state));
    applyClaimCredentials(result);
    if (mode === 'remote' && result.state) {
      applyRemoteAppState(result.state, result);
    } else {
      state = localSnapshot;
      applyClaimCredentials(result);
    ...
    if (state.profile.gender || state.profile.salutation) {
      try {
        const payload = telegramSettingsPayload(false);
        const settingsResult = await telegramApi('/api/telegram/settings', { method: 'PUT', body: payload });
        applyTelegramSettings(payload, settingsResult);
      } catch (error) {
        console.warn('Telegram profile sync after login failed', error);
      }
    }
    renderDiary();
    renderProfile();
    ...
    closeModal();
    telegramHaptic('success');
    setTimeout(() => showTelegramAuthorizationSuccess({ source: 'pwa', force: true }), 120);
  }

  async function claimTelegramSession(code) {
    const normalized = String(code || '').replace(/[^A-Za-z0-9]/g, '').toUpperCase();
    if (!/^[A-Z0-9]{8}$/.test(normalized)) {
      toast('Введите восьмизначный код из Telegram');
      return;
    }
    showSessionGate('Подключаем PWA к вашему Telegram…');
    try {
      const response = await fetch('/api/telegram/claim', {
        method: 'POST',
        headers: { 'content-type': 'application/json', accept: 'application/json' },
        body: JSON.stringify({ code: normalized, device: getDeviceIdentity('pwa') })
      });
      const result = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(result.error || 'Не удалось подключить Telegram');

      const localHasData = hasMeaningfulLocalData(state);
      if (result.state && localHasData) {
        hideSessionGate();
        openModal(`<div class="sheet__handle"></div>
          <div class="sheet__header"><div><h2>Выберите <em>данные</em></h2><p>Telegram-сессия найдена. Решите, какой дневник должен стать общим.</p></div></div>
          <div class="sheet__body">
            <button id="useTelegramData" class="sync-choice" type="button"><strong>Использовать данные из Telegram</strong><small>Заменит данные на этом устройстве облачным дневником.</small></button>
            <button id="movePhoneData" class="sync-choice" type="button"><strong>Перенести данные этого телефона</strong><small>Текущий профиль и дневник станут общими для PWA и Mini App.</small></button>
            <p class="reminder-note">Выбор применяется только после нажатия. Код уже подтверждён и больше не работает повторно.</p>
          </div>`);
        $('#useTelegramData').addEventListener('click', () => finalizeTelegramClaim(result, 'remote'));
        $('#movePhoneData').addEventListener('click', () => finalizeTelegramClaim(result, 'local'));
      } else {
        await finalizeTelegramClaim(result, result.state ? 'remote' : 'local');
      }
    } catch (error) {
      toast(error.message || 'Ошибка входа через Telegram');
    } finally {
      hideSessionGate();
    }
  }
  ...
  function openTelegramReminders() {
    if (!state.telegram.authToken || !state.telegram.connected) {
      openTelegramFirstLogin();
      return;
    }
    ...
    openModal(`<div class="sheet__handle"></div>
      ...
      <div class="sheet__body">
        ...
        <div class="sheet__actions"><button id="reconnectTelegramDevice" class="secondary-button" type="button">Переподключить PWA</button><button id="saveTelegramSettings" class="primary-button" type="button">Сохранить</button></div>
        ${t.botUsername && t.connected ? `<button id="openSameTelegramSession" class="secondary-button" style="width:100%;margin-top:9px" type="button">Открыть Mini App</button>` : ''}
        <button id="logoutTelegramDevice" class="danger-button" style="width:100%;margin-top:9px" type="button">Выйти из Telegram на этом устройстве</button>
        ${t.sessionId ? `<p class="reminder-note">ID общей сессии: <code>${escapeHtml(t.sessionId.slice(0, 8))}</code></p>` : ''}
      </div>`);
    ...
    $('#reconnectTelegramDevice')?.addEventListener('click', () => {
      ...
      storeStateSilently();
      closeModal();
      setTimeout(openTelegramFirstLogin, 100);
    });
    $('#logoutTelegramDevice')?.addEventListener('click', () => {
      state.telegram = { ...defaultState().telegram, botUsername: state.telegram.botUsername };
      storeStateSilently();
      stopTelegramAuthorizationWatch();
      clearInterval(statePullInterval);
      closeModal();
      renderTelegramStatus();
      toast('Выход выполнен только на этом устройстве');
    });
    $('#saveTelegramSettings').addEventListener('click', async () => {
      ...
      try {
        ...
        await pushAppState({ quiet: true });
        closeModal();
        toast('Расписание и общая сессия Telegram сохранены');
      } catch (error) {
        toast(error.message);
      }
    });
    refreshTelegramStatus({ quiet: true, announce: false }).then((status) => {
      if (status) updateTelegramStatusCard(status);
    });
    if (t.awaitingAuthorization && !t.connected) startTelegramAuthorizationWatch();
  }
  ...
  function initializeEvents() {
    ...
    $('#scanPhoto').addEventListener('click', () => $('#scannerPhotoInput').click());
    $('#scannerPhotoInput').addEventListener('change', async (event) => {
  ...
  function showSessionGate(message = 'Синхронизируем данные…') {
    let gate = document.getElementById('sessionSyncGate');
    if (!gate) {
      gate = document.createElement('div');
      gate.id = 'sessionSyncGate';
      gate.className = 'session-sync-gate';
      gate.innerHTML = '<div class="session-sync-gate__card"><i></i><strong></strong><small>Не закрывайте окно несколько секунд</small></div>';
      document.body.appendChild(gate);
    }
    gate.querySelector('strong').textContent = message;
    gate.classList.add('is-visible');
  }

  function hideSessionGate() {
    document.getElementById('sessionSyncGate')?.classList.remove('is-visible');
  }

  function startSessionPolling() {
    clearInterval(statePullInterval);
    if (!state.telegram.authToken) return;
    statePullInterval = setInterval(() => {
      if (document.visibilityState === 'visible') pullAppState({ quiet: true });
    }, 12000);
  }

  function initializeSessionRefreshEvents() {
    const refresh = () => {
      if (state.telegram.authToken && document.visibilityState === 'visible') {
        refreshTelegramStatus({ quiet: true, announce: Boolean(state.telegram.awaitingAuthorization) });
        pullAppState({ quiet: true });
      }
    };
  ...
  async function initializeTelegram() {
    ...
    try {
      ...
      return await initializeTelegramMiniAppSession();
    } catch (error) {
  ...
  async function start() {
    initializeEvents();
    initializeSessionRefreshEvents();
    initializePwa();
    loadTelegramPublicConfig();
    renderDiary();
    renderProfile();
    renderFoodCatalog();
    navigate('diary');
    const openedFromTelegram = await initializeTelegram();
    if (!openedFromTelegram && state.telegram.authToken) {
      await refreshTelegramStatus({ quiet: true, announce: Boolean(state.telegram.awaitingAuthorization) });
      await pullAppState({ quiet: true });
      startSessionPolling();
