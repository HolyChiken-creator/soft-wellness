(() => {
  ...
  const STORAGE_KEY = 'softWellnessStateV7';
  const LEGACY_STORAGE_KEYS = ['softWellnessStateV6', 'softWellnessStateV5', 'softWellnessStateV4'];
  const APP_VERSION = 7;
  const MEALS = [
  ...
  async function initializeTelegramMiniAppSession() {
    ...
    try {
      const startParam = webApp?.initDataUnsafe?.start_param || new URLSearchParams(location.search).get('tgWebAppStartParam') || '';
      ...
      renderTelegramStatus();
      toast('Открыта общая сессия PWA и Telegram');
      return true;
    } catch (error) {
  ...
  function openTelegramReminders() {
    ...
    openModal(`<div class="sheet__handle"></div>
      ...
      <div class="sheet__body">
        ...
        ${t.botUsername ? `<a class="secondary-button" style="display:flex;align-items:center;justify-content:center;text-decoration:none;margin-top:9px" href="https://t.me/${escapeHtml(t.botUsername)}?startapp=sync" target="_blank" rel="noreferrer">Открыть ту же сессию в Telegram</a>` : ''}
      </div>`);
