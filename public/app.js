(() => {
  ...
  const STORAGE_KEY = 'softWellnessStateV9';
  const LEGACY_STORAGE_KEYS = ['softWellnessStateV8', 'softWellnessStateV7', 'softWellnessStateV6', 'softWellnessStateV5', 'softWellnessStateV4'];
  const APP_VERSION = 9;
  const MEALS = [
  ...
  function renderTelegramStatus() {
    ...
    else if (state.telegram.awaitingAuthorization) element.textContent = '⏳ Ожидаем подтверждение в Telegram';
    else if (state.telegram.authToken) element.textContent = 'Код создан · откройте бота';
  ...
  function telegramStatusCopy(status = state.telegram) {
    if (status.connected) {
      return {
        ...
        description: 'PWA и Telegram Mini App используют одну облачную сессию.',
        className: 'is-connected'
      };
    }
    if (state.telegram.awaitingAuthorization) {
      return {
        title: '⏳ Ждём подтверждение в Telegram',
        description: 'Отправьте боту команду /start с кодом. Этот экран обновится автоматически.',
        className: 'is-pending'
      };
    }
    return {
      title: 'Telegram ещё не подключён',
      description: 'Создайте код и отправьте его боту командой /start.',
      className: ''
    };
  }
  ...
  function showTelegramAuthorizationSuccess({ source = 'pwa', force = false } = {}) {
    ...
    const sourceText = source === 'telegram'
      ? 'Вы вошли через Telegram Mini App. Загружена та же сессия, что используется в PWA.'
      : 'Бот подтвердил подключение. PWA и Telegram Mini App теперь используют один дневник.';
  ...
  async function initializeTelegramMiniAppSession() {
    ...
    showSessionGate('Подключаем ту же сессию, что и в PWA…');
    try {
      const rawStartParam = webApp?.initDataUnsafe?.start_param
        || new URLSearchParams(location.search).get('tgWebAppStartParam')
        || '';
      const sessionTicket = new URLSearchParams(location.search).get('sw_session')
        || String(rawStartParam).replace(/^sw[_-]?/i, '');
      const response = await fetch('/api/telegram/session', {
        method: 'POST',
        headers: { 'content-type': 'application/json', accept: 'application/json' },
        body: JSON.stringify({ initData, startParam: rawStartParam, sessionTicket })
      });
      ...
      if (result.state) {
        ...
        const pulled = await pullAppState({ force: true, quiet: false });
        if (!pulled?.state) await pushAppState({ quiet: false });
      }
      renderTelegramStatus();
      startSessionPolling();
      hideSessionGate();
  ...
  function openTelegramReminders() {
    ...
    openModal(`<div class="sheet__handle"></div>
      ...
      <div class="sheet__body">
        ...
        ${t.botUsername && t.connected ? `<button id="openSameTelegramSession" class="secondary-button" style="width:100%;margin-top:9px" type="button">Открыть именно эту сессию в Telegram</button>` : ''}
    ...
    const registerOrRefresh = async () => {
      ...
      try {
        ...
        applyTelegramSettings(payload, result);
        state.telegram.awaitingAuthorization = true;
        state.telegram.authorizationAcknowledgedSessionId = '';
        persist(false);
        await pushAppState({ quiet: false });
        closeModal();
        toast('Код создан. Ждём подтверждение от Telegram…');
        setTimeout(() => {
          openTelegramReminders();
          startTelegramAuthorizationWatch();
        }, 180);
      } catch (error) {
        toast(error.message);
      }
    ...
    $('#saveTelegramSettings').addEventListener('click', async () => {
      ...
      try {
        if (!state.telegram.authToken) {
          ...
          await pushAppState({ quiet: false });
          closeModal();
          toast('Код создан. Подтвердите вход командой /start в боте');
          setTimeout(() => {
            openTelegramReminders();
            startTelegramAuthorizationWatch();
          }, 180);
