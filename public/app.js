(() => {
  ...
  const STORAGE_KEY = 'softWellnessStateV10';
  const LEGACY_STORAGE_KEYS = ['softWellnessStateV9', 'softWellnessStateV8', 'softWellnessStateV7', 'softWellnessStateV6', 'softWellnessStateV5', 'softWellnessStateV4'];
  ...
  function renderTelegramStatus() {
    ...
    if (!element) return;
    if (state.telegram.connected && state.telegram.enabled) element.textContent = '✅ Авторизация подтверждена · напоминания активны';
    else if (state.telegram.connected) element.textContent = '✅ Авторизация подтверждена · напоминания приостановлены';
  ...
  async function initializeTelegramMiniAppSession() {
    ...
    try {
      const response = await fetch('/api/telegram/session', {
        ...
        body: JSON.stringify({ initData, appUrl: location.origin })
      });
  ...
  async function openTelegramFirstLogin() {
    ...
    openModal(`<div class="sheet__handle"></div>
      <div class="sheet__header"><div><h2>Вход через <em>Telegram</em></h2><p>Теперь аккаунт создаёт бот. PWA только подключается к уже существующей Telegram-сессии.</p></div><button class="sheet__close" type="button" data-close-modal>×</button></div>
      <div class="sheet__body">
        <div class="telegram-first-flow"><div><i>1</i><p><strong>Откройте бота</strong><small>Нажмите Start или отправьте /code.</small></p></div><div><i>2</i><p><strong>Скопируйте код</strong><small>Он состоит из восьми символов и действует 10 минут.</small></p></div><div><i>3</i><p><strong>Введите его ниже</strong><small>Эта PWA войдёт в ту же сессию, что и Mini App.</small></p></div></div>
        ...
        <p class="reminder-note">Код создаётся со стороны Telegram. Никаких отдельных PWA-профилей на сервере больше не создаётся.</p>
      </div>`);
